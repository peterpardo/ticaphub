...
<body>
    <div x-data="setup()" :class="">
        <div
            ...
            {{-- Sidebar --}}
            <div
                class="fixed flex flex-col top-14 left-0 w-14 hover:w-64 md:w-64 bg-red-900 dark:bg-gray-900 h-full text-white transition-all duration-300 border-none z-10 sidebar">
                    ...
                    <ul class="flex flex-col py-4 space-y-1 overflow-y-hidden">
                        ...
                        <li>
                            <a href="{{ route('dashboard') }}"
                                class="relative flex flex-row items-center h-11 focus:outline-none hover:bg-red-800 dark:hover:bg-gray-600 text-white-600 hover:text-white-800 border-l-4 border-transparent hover:border-red-500 dark:hover:border-gray-800 pr-6">
                        ...
                        @can('access users')
                            <x-app.sidebar-link name="User Accounts" route="{{ route('users') }}">
                                <x-slot name="icon">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    Auth::user()->committee()->exists())
                            <li>
                                @if (Auth::user()->committeeMember()->exists())
                                    <a href="/committee/{{ Auth::user()->committeeMember->committee->id }}"
                                        class="relative flex flex-row items-center h-11 focus:outline-none hover:bg-red-800 dark:hover:bg-gray-600 text-white-600 hover:text-white-800 border-l-4 border-transparent hover:border-red-500 dark:hover:border-gray-800 pr-6">
                                    @elseif(Auth::user()->committee()->exists())
                                        <a href="/committee/{{ Auth::user()->committee->id }}"
                                            class="relative flex flex-row items-center h-11 focus:outline-none hover:bg-red-800 dark:hover:bg-gray-600 text-white-600 hover:text-white-800 border-l-4 border-transparent hover:border-red-500 dark:hover:border-gray-800 pr-6">
                                @endif
                                <span class="inline-flex justify-center items-center ml-4">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                ...
                                @if (Auth::user()->committeeMember()->exists())
                                    <span
                                        class="ml-2 text-sm tracking-wide truncate">{{ Auth::user()->committeeMember->committee->name }}
                                        Committee</span>
                                @elseif(Auth::user()->committee()->exists())
                                    <span
                                        class="ml-2 text-sm tracking-wide truncate">{{ Auth::user()->committee->name }}
                                        Committee</span>
                                @endif
                                </a>
                            </li>
                        ...
                        @can('access group exhibit')
                            <li>
                                <a href="{{ route('exhibit') }}"
                                    ...
                                    <span
                                        class="ml-2 text-sm tracking-wide truncate">{{ Auth::user()->userGroup->group->name . ' Exhibit' }}</span>
                                </a>
                            </li>
                        @endcan
                        ...
                        @role('admin')
                            <div class="overflow-y-hidden overflow-x-hidden flex flex-col justify-between flex-grow">
                                <button
                                    class="relative flex flex-row items-center h-11 focus:outline-none hover:bg-red-800 dark:hover:bg-gray-600 text-white-600 hover:text-white-800 border-l-4 border-transparent hover:border-red-500 dark:hover:border-gray-800 pr-6"
                                    type="button" data-dropdown-toggle="dropdown"><span
                                ...
                                <div class="hidden text-base z-50 list-none divide-y divide-gray-100 rounded shadow my-4"
                                    id="dropdown">
                                    <ul class="py-1" aria-labelledby="dropdown">
                                        <li>
                                            <a href="{{ route('home.slider') }}"
                                                class="bg-red-800 dark:bg-gray-900 relative flex flex-row items-center h-11 focus:outline-none hover:bg-red-800 dark:hover:bg-gray-600 text-white-600 hover:text-white-800 border-l-4 border-transparent hover:border-red-500 dark:hover:border-gray-800 pr-6">
                                                <span class="inline-flex justify-center items-center ml-4">
