<div>
    ...
    <div class="w-full mb-8 overflow-hidden rounded-lg shadow-lg">
        <table class="w-full table-fixed">
            ...
            <tr class="text-gray-700">
                <td class="px-4 py-3 text-xl font-bold border">{{ $spec->name . ' (' . $spec->school->name . ')'}}</td>
                <td class="px-4 py-3 text-md border">
                    ...
                    @else
                        <table class="table-fixed">
                            ...
                            <tr>
                                <td class="px-2 py-1">{{ $panelist->user->first_name }} {{  $panelist->user->middle_name }} {{ $panelist->user->last_name }}</td>
                                <td class="px-2 py-1"><button wire:click="deletePanelist({{ $panelist->user->id }})" class="bg-red-500 hover:bg-red-600 text-white px-2 rounded">&times;</button></td>
                            </tr>
                            @endforeach
                        </table>
                    @endif
    ...
    <div class="hidden min-w-screen h-screen animated fadeIn faster  fixed  left-0 top-0 justify-center items-center inset-0 z-50 outline-none focus:outline-none" id="deletePanelistModal">
