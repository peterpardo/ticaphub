<div>
    @if ($users->count() > 0)
        <div class="flex justify-between mb-3">
            <a href="/officers-and-committees/positions"
                class="inline-block bg-red-500 px-10 py-2 rounded text-white">Back</a>
            <button wire:click="confirmCandidates"
                class="inline-block bg-green-600 py-2 px-10 rounded text-white hover:bg-green-500">Start
                Election</button>
        </div>
    @endif
    ...
    <div class="mb-2 text-gray-800">
        <div class="flex justify-between">
            <div class="flex-1">
                ...
                <input type="text" class="rounded my-2" placeholder="Search student name" autocomplete="off"
        ...
        @enderror
        @if (session('status'))
            <div class="bg-{{ session('status') }}-100 border-l-4 border-{{ session('status') }}-500 text-{{ session('status') }}-700 p-4"
                role="alert">
                    ...
                    @foreach ($users as $user)
                        <tr class="text-gray-700">
                            <td class="px-4 py-1 border">
                                <div class="flex items-center text-sm">
                                    <div class="relative w-8 h-8 mr-3 rounded-full md:block">
                                        ...
                                        @else
                                            <img class="object-cover w-full h-full rounded-full"
                                                src="{{ url(asset('assets/default-img.png')) }}" alt=""
                                                loading="lazy" />
                                        @endif
                            ...
                            <td class="px-4 py-3 text-md font-semibold border">
                                {{ $user->userSpecialization->specialization->school->name }}</td>
                            <td class="px-4 py-3 text-md border">
                                {{ $user->userSpecialization->specialization->name }}</td>
                            <td class="px-4 py-3 text-md border text-center">
                                <button wire:click="addCandidate({{ $user->id }})"
                                    class="w-1/2 rounded shadow px-2 py-1 text-white bg-blue-500 hover:bg-blue-600">Add</button>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
                    ...
                    <tbody class="bg-white">
                        @foreach ($candidates as $candidate)
                            <tr class="text-gray-700">
                                    ...
                                    <div class="flex items-center text-sm">
                                        <div class="relative w-8 h-8 mr-3 rounded-full md:block">
                                            @if ($candidate->user->profile_picture)
                                                <img class="object-cover w-full h-full rounded-full"
                                                    src="{{ Storage::url($candidate->user->profile_picture) }}"
                                                    alt="" loading="lazy" />
                                            @else
                                                <img class="object-cover w-full h-full rounded-full"
                                                    src="{{ url(asset('assets/default-img.png')) }}" alt=""
                                ...
                                <td class="px-4 py-3 text-md font-semibold border">{{ $candidate->election->name }}
                                </td>
                                <td class="px-4 py-3 text-md font-semibold border">{{ $candidate->position->name }}
                                </td>
                                <td class="px-4 py-3 text-md border text-center">
                                    <button wire:click="deleteCandidate({{ $candidate->id }})"
                                        class="rounded shadow px-2 py-1 text-white bg-red-500 hover:bg-red-600">Remove</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
            ...
            <div class="text-gray-800">
                ...
                <div class="text-center p-5 flex-auto justify-center">
                    <h2 class="text-xl font-bold py-4 ">Are you sure?</h3>
                        <p class="text-sm text-gray-500 px-8 mb-5">Do you want to start the Election with these
                            settings? You will not be able to change it once you proceed.</p>
