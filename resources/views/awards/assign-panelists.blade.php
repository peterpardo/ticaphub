<x-app-layout :scripts="$scripts">
    ...
    <form action="/set-panelist/assign" method="post" id="panelistForm">
        @csrf
        <div class="bg-white text-gray-800 w-2/5 px-3 py-4 mx-auto rounded shadow">
            ...
            <div class="mb-2">
                ...
                @enderror
                <div id="message"></div>
                <div class="bg-white w-full mb-8 overflow-hidden">
                    <div class="w-full overflow-x-auto">
                        <table class="w-full table-auto" id="panelistTable">
                            <tr class="text-center text-md font-semibold tracking-wide text-gray-900 uppercase">
                                <td>
                                    <select name="panelists[]" class="w-full">
                                        <option value="">--- select panelist ---</option>
                                        @foreach ($panelists as $panelist)
                                            <option value="{{ $panelist->id }}">{{ $panelist->first_name }}
                                                {{ $panelist->middle_name }} {{ $panelist->last_name }}</option>
                                        @endforeach
                                    </select>
                                </td>
                            </tr>
            ...
            <a id="addBtn"
                class="inline-block cursor-pointer bg-blue-500 hover:bg-blue-600 rounded text-white px-2 py-2">+
                panelist</a>
            <div class="text-center">
                <a href="/set-panelist" class="inline-block shadow rounded text-black px-2 py-2 mr-3">Cancel</a>
                <button type="submit"
                    class="bg-green-500 hover:bg-green-600 rounded text-white px-2 py-2">Submit</button>
            </div>
        </div>
    </form>
</x-app-layout>
